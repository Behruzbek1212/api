...
class JobServices
{
    ...
    public function createJobBanner($company, $title, $salary, $address, $postNumber = 0, $bonus = false)
    {
        ...
        $jpgImage->text($postNumber, 1010, 390, function ($font) use ($fontPath) {
            $font->file($fontPath);
            $font->size(70);
            $font->color('#057AF5');
            $font->align('center');
        ...
        $jpgImage->text($text2, 425, 1410, function ($font) use ($gilroyLight) {
        ...
        $jpgImage->text($text3, 1370, 1410, function ($font) use ($gilroyLight) {
            $font->file($gilroyLight);
            $font->size(47);
            $font->color('#474747');
            $font->align('left');
